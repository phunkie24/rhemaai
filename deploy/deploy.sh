...
sed "s/DOMAIN/${DOMAIN}/g" "$APP_DIR/deploy/security-headers.nginx" > /etc/nginx/snippets/rhemaai-security.conf
sed "s/DOMAIN/${DOMAIN}/g" "$APP_DIR/deploy/nginx.conf" > /etc/nginx/sites-available/rhemaai
