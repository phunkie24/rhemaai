...
describe('Nexus pricing page', () => {
  it('renders the essential public prices without offline commercial detail', () => {
    ...
    expect(screen.getAllByText('From ₦2,500,000 / $3,500').length).toBeGreaterThan(0)
    expect(screen.getAllByText('From ₦7,500,000 / $20,000').length).toBeGreaterThan(0)
    expect(screen.getAllByText('From ₦60,000,000 / $150,000').length).toBeGreaterThan(0)
    expect(screen.getByRole('heading', { name: 'Architecture Blueprint' })).toBeInTheDocument()
    expect(screen.getByRole('heading', { name: '24/7 Mission-Critical AgentOps' })).toBeInTheDocument()
